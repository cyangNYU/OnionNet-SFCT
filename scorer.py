...
def rescoring(receptor_file, ligand_file, output, model_pkl, method='rabind-rf',
    ...
    for i, mol_id in enumerate(mol2_code_list):
        ...
        if os.path.exists(reference):
            ...
            tofile.write("{} {} {:.3f} {:.3f} {:.3f} {:.3f}\n".format(mol2_code_list[i], i, pose_scores[i], score, ml_scores[i], rmsd))
        else:
            tofile.write("{} {} {:.3f} {:.3f} {:.3f}\n".format(mol2_code_list[i], i, pose_scores[i], score, ml_scores[i]))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-l', type=str, help="Ligand filename.Support format: mol2, sdf, pdb or pdbqt format.")
    parser.add_argument('--ref', default="refer.mol2", type=str, help="(Optional, no required) Reference Ligand mol2 filename.")
